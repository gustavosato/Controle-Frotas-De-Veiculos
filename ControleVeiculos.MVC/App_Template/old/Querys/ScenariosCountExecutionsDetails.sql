...
--, avg(convert(float,[Amount Runs]) / convert(float,[Amount Tests])) as [Average Number of Scenario Executions]
from #@Temp1
Group By Demand, Scenario, [Amount Tests], [Amount Runs]

select demand, Scenario as [Total of Scenarios], [Amount Tests] as [Total of Tests], [Amount Runs] as [Total of Runs],
case when [Amount Runs] = 0 then
	0
else
	round(avg(convert(float,[Amount Runs]) / convert(float,[Amount Tests])), 2)
end
as [Average Number of Scenario Executions]
from #@Temp2
